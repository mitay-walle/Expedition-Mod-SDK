...
{
    [Serializable, NoAutoStaticsCleanup]
    [UnityEngine.Scripting.APIUpdating.MovedFrom(true, sourceAssembly: "Survival")]
    public struct GridShape : ISerializationCallbackReceiver, IEquatable<GridShape> //, IEquatable<Shape>
    {
        ...
        public GridShape(int x, int y, IEnumerable<Vector2Int> skippedPositions = null, bool readable = false) : this(skippedPositions, readable) =>
            _size = new(x, y);

        public GridShape(Vector2Int size, IEnumerable<Vector2Int> skippedPositions = null, bool readable = false) :
            this(skippedPositions, readable) => _size = size;
        ...
        public void GetValidPositions(List<Vector2Int> buffer, bool invertY = false, bool invertValid = false)
        {
            buffer.Clear();
            ...
            for (uint y = 0; y < _size.y; y++)
            {
                ...
                {
                    if (_bitArray[IndexFromPosition(x, y)] != invertValid)
                    {
        ...
        private bool[,] PreviewValues
        {
            get
            {
                var values = new bool[_size.x, _size.y];
                for (int y = 0; y < _size.y; y++)
                {
                    for (int x = 0; x < _size.x; x++)
                    {
                        values[x, y] = _bitArray[IndexFromPosition(x, y, true)];
                    }
                }
                return values;
            }
            set
            {
                for (int y = 0; y < _size.y; y++)
                {
                    for (int x = 0; x < _size.x; x++)
                    {
                        _bitArray[IndexFromPosition(x, y, true)] = value[x, y];
                    }
                }
            }
        }
        ...
        private static bool Draw(Rect rect, bool value)
        {
            ...
            EditorGUI.DrawRect(new Rect(rect.x + 1, rect.y + 1, rect.width - 2, rect.height - 2),
                value ? Color.green : new Color(0.15f, 0.15f, 0.15f));
            return value;
        }
#endif
        ...
        {
            return _size.Equals(other._size) && _bitArray.Equals(other._bitArray);
        }

        public override bool Equals(object obj)
        {
            return obj is GridShape other && Equals(other);
        }

        public override int GetHashCode()
        {
            return HashCode.Combine(_size, _bitArray);
        }
    }
}